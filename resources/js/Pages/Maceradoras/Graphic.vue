...
props.data.map( (row) =>(
    clientes.value.push(row.institucion),
    maceradoras.value.push(row.count),
    colors.value.push("rgb("+random()+","+random()+","+random()+")")
))
chartOptions.value= { responsive:true}
chartData.value = {
    labels:clientes.value,
    datasets:[
        {label:'Maceradoras', data:maceradoras.value, backgroundColor:colors}
    ]
}
</script>
